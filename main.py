...
def parse_opt():
    ...
    parser.add_argument(
        "--source-file", type=str, default=ROOT / "result.txt", help="result file"
    )
    parser.add_argument(
        "--Manually-annotate-dir", type=str, default=ROOT / "gt", help="annotate dir"
    )
    parser.add_argument(
        "--save-csv-path",
        type=str,
        default=ROOT / "result" / "result.csv",
    ...
    parser.add_argument(
        "--imgsz", "--img", "--img-size", type=int, default=1920, help="img size"
    )
    parser.add_argument("--img-path", type=str, default=ROOT / "test", help="img path")
    parser.add_argument(
        "--save-img-path",
        type=str,
        default=ROOT / "result" / "save_pic",
        help=" save img path",
    )
    parser.add_argument(
        "--conf-thres-setting",
        action="store_false",
        help=" Threshold 0-1 interval 0.001",
    )
...
def main(opt):
    ...
    ##abs path
    opt.source_file = (
        opt.source_file.replace(".\\", os.getcwd() + os.sep, 1)
        if str(opt.source_file).startswith(".\\")
        else opt.source_file
    )
    opt.Manually_annotate_dir = (
        opt.Manually_annotate_dir.replace(".\\", os.getcwd() + os.sep, 1)
        if str(opt.Manually_annotate_dir).startswith(".\\")
        else opt.Manually_annotate_dir
    )
